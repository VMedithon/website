...
|| GAP-IMPORT-01 | Devnovate import | Generic CSV mapping implemented with flexible column detection (team, title, track, members/emails/names); processes in queue to create users, teams, team members, and pitch submissions | Validate against a real Devnovate export and refine mapping if columns differ | External submission data may not import cleanly without verification | Phase 7 |
...
|| GAP-TEST-02 | Integration tests | Unit tests for state machines, utils, and validation; Playwright E2E for landing, health, and certificate verification using Brave | Worker integration tests with `@cloudflare/vitest-pool-workers` and Playwright E2E per `testing-strategy.md` | Regressions ship silently | Phase 1+ |
|| GAP-RENDER-01 | Certificate render | SVG artifacts generated with embedded background images; public verification returns HMAC-SHA256 signature; certificate file available at `/api/public/certificates/:id/file`; browser-side PNG download available from the verify page; server-side PDF/PNG render is not required | PDF/PNG render and signed verification payload honoring `COMP-VERIFY-01` | Printed certificates lack credibility | Phase 6 |
